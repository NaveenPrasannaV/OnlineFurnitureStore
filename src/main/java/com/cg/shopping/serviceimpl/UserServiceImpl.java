...
public class UserServiceImpl implements UserService {
	 ...
	 * other methods in this class. Check the values and the existance of the
	 * database.
	 */
	@Override
	 ...
	 * method of User Dao. It will signout the previous user, if any user already
	...
	public String userSignIn(String userEmail, String userPassword) throws NoUserFoundException {
		if (userDao.isExistsByUserEmailAndUserPassword(userEmail, userPassword)) {
			...
			findCartByUserEmail(UserServiceImpl.userEmail);
	...
	/*
	 * This method will signout the user from the application service.
	 */
	@Override
	 ...
	 * common method.
	 */
	public String addProductToGivenCart(Cart cart, int productId) {
		...
		if (userSignInVerification()) {
			if (productDao.existsByProductId(productId) && productDao.getProductQty(productId) > 0) {
				...
				if (ShoppingValidation.cartProductCountValidation(cart, productId) == 0) {
					return addProductToGivenCart(cart, productId);
				}
				return "Product already in the cart, You can increase the Qty in the cart";
			}
	 ...
	 */
	public String removeProductFromGivenCart(Cart cart, int productId) {
		...
		if (userSignInVerification()) {
			...
			if (productQty > 0) {
				for (int i = 0; i < productQty; i++) {
					msg = removeProductFromGivenCart(cart, productId);
				}
				return msg;
			}
	...
	public String changeProductQuantityInCart(int productId, int qty)
		...
		if (userSignInVerification()) {
			...
			if (qty > 0 && productQty >= qty) {
				for (int i = 0; i < qty; i++) {
					msg = addProductToGivenCart(cart, productId);
				}
				return msg;
	...
	/*
	 * This method will search a cart for an user. If the cart is not allocated then
	 * it will create one cart and allocate it to that user
	 */
